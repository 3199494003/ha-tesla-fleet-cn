...
| `lib/tesla_fleet_api/` | **vendored 上游 tesla-fleet-api 1.10.1 + 中国区 token 端点补丁**：上游只把授权链接(登录)改到 `auth.tesla.cn`，但 `partner_login`/token 交换三处仍写死国际 `fleet-auth.prd.vn.cloud.tesla.com`（中国区应用会被拒 `The request body is not valid`）。补丁按区域选择 token 端点（中国区= `auth.tesla.cn/oauth2/v3/token`，经 OIDC 元数据核实）；pb2 生成代码走独立的 `tesla-protocol` 包（gencode 6.31.x，兼容 HA 内置 protobuf 6.32） |
